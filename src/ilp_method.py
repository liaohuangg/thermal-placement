...
根据新的数学规划模型：
- 相邻约束：有连接的模块必须紧靠在一起（水平或垂直相邻）
- 旋转约束：使用二进制变量 r_k 表示是否旋转90度
- 目标函数：最小化有连接关系的芯片之间的中心距离

使用 PuLP + GLPK 求解器实现。

核心思想：
对于每对有连接的模块对 (A, B)，它们必须满足以下条件之一：
1. 水平相邻：A 和 B 左右紧靠，且垂直方向有重叠
2. 垂直相邻：A 和 B 上下紧靠，且水平方向有重叠

使用二进制变量控制相邻方式，使用大M方法实现条件约束。
"""
...
def solve_placement_ilp(
    ...
    """
    使用ILP求解芯片布局问题（相邻约束模型）。
    
    核心约束：
    1. 对于每对有连接的模块对，它们必须相邻（紧靠）
    2. 相邻方式：水平相邻或垂直相邻（二选一）
    3. 水平相邻：左右紧靠，垂直方向的共享边长度必须 >= min_shared_length
    4. 垂直相邻：上下紧靠，水平方向的共享边长度必须 >= min_shared_length
    5. 所有模块必须在边界框内，不超出芯片边界
    6. 所有模块对都不能重叠（即使没有连接）
    
    参数
    ----
    nodes:
        Chiplet节点列表（从JSON文件读入）
    edges:
        边列表，表示有互联关系的节点对（这些节点对必须相邻）
    W:
        芯片边界框宽度 ChipW（如果不指定，自动计算）
    H:
        芯片边界框高度 ChipH（如果不指定，自动计算）
    time_limit:
        求解时间限制（秒）
    verbose:
        是否打印详细信息
    min_shared_length:
        相邻chiplet之间共享边的最小长度（单位：与模块尺寸相同的单位）。
        例如：
        - min_shared_length = 0.0：相邻chiplet只需要有接触即可（共享边长度 >= 0）
        - min_shared_length = 1.0：相邻chiplet的共享边长度必须 >= 1.0
        - min_shared_length = 2.0：相邻chiplet的共享边长度必须 >= 2.0
        对于水平相邻：共享边是垂直方向的接触长度
        对于垂直相邻：共享边是水平方向的接触长度
        默认值为 0.0（只需要有接触）。
    minimize_bbox_area:
        是否最小化外接方框面积。默认值为 True。
    distance_weight:
        目标函数中距离项的权重。默认值为 1.0。
    area_weight:
        目标函数中外接方框“面积代理”项的权重。默认值为 0.1。
        当前实现采用 AM-GM 不等式的凸近似：
        - 通过变量 t 以及约束 t ≤ (bbox_w + bbox_h)/2,  bbox_w ≤ K t, bbox_h ≤ K t
        - 在目标函数中最小化 t，从而同时压缩 bbox_w 和 bbox_h
        这样保持了目标函数与约束的线性形式，便于 ILP 求解。
