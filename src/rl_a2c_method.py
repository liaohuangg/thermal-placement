...
def train_a2c(
    ...
    print(f"开始训练A2C: state_dim={state_dim}, max_action_dim={max_action_dim}")
    ...
    for episode in range(num_episodes):
        ...
        if len(rewards) > 0:
            ...
            # 从后往前计算回报和advantage
            gae = torch.tensor(0.0).to(device)  # Generalized Advantage Estimation
            ...
            # 为每个状态计算动作概率（只考虑实际执行的动作）
            # 由于我们存储的是实际执行的动作，直接计算这些动作的对数概率
            action_log_probs_all = F.log_softmax(action_logits, dim=-1)
            selected_log_probs = action_log_probs_all.gather(1, actions_t.unsqueeze(1)).squeeze(1)
            
            # Actor损失：-log π(a|s) * Advantage
            actor_loss = -(selected_log_probs * advantages_t).mean()
            
            # 计算熵（用于正则化）- 需要重新计算以考虑有效动作
            # 简化处理：使用所有动作的熵
