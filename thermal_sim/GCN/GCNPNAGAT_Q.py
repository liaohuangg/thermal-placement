    ...
    if not is_inference:
        ...
        for epoch in range(epoch_mem,30000):
            ...
            for i in range(0, num_train, batch_size):  
                ...
                if epoch == epoch_mem and i == 0:
                    print("[First Batch] Loss: {:.6f}, RMSE: {:.6f}, MAE: {:.6f}".format(
                        loss.item(), acc_current.item(), MAE.item()*(Temperature_max - Temperature_min)))
            ...
            print("Train Epoch {:05d} | Time(s) {:.4f} | Loss {:04f} | RMSE {:.4f} | MAE {:.4f} | AEmax {:.4f}".format(
                epoch, epoch_time, epoch_loss, epoch_acc, epoch_MAE*(Temperature_max - Temperature_min), 
                epoch_AEmax*(Temperature_max - Temperature_min)))
            ...
            if epoch % 1 == 0:
                ...
                print("Test Epoch {:05d} | RMSE {:.4f} | MAE {:.4f} | AEmax {:.4f} | Test time: {:.4f}s".format(
                    epoch, epoch_test_acc, epoch_test_MAE*(Temperature_max - Temperature_min), 
                    epoch_test_AEmax*(Temperature_max - Temperature_min), test_time))
                with open(results_dir + 'train_acc.txt','a') as Train_Acc_file:
        ...
        print("=" * 60)
        print("Loading model from: {}".format(ckpt_file))
        inference_start_time = time.time()
        
        model.load_state_dict(torch.load(ckpt_file, map_location=device))
        model.to(device=device)
        
        load_time = time.time() - inference_start_time
        print("Model loaded successfully in {:.4f}s".format(load_time))
        ...
        for i in range(0, num_test, batch_size):
            ...
            print("Test batch {} | RMSE {:.4f} | MAE {:.4f} | Max {:04f} | Min {:.4f} | Time: {:.4f}s".format(
                i, acc, MAE*(Temperature_max-Temperature_min), AEmax*(Temperature_max-Temperature_min), 
                AEmin*(Temperature_max-Temperature_min), batch_time))
        ...
        print("  Total RMSE: {:.4f}".format(epoch_test_acc))
        print("  Average MAE: {:.4f}".format(epoch_test_MAE*(Temperature_max - Temperature_min)))
        print("  Max Error: {:.4f}".format(epoch_test_Max*(Temperature_max-Temperature_min)))
        print("  Min Error: {:.4f}".format(epoch_test_Min*(Temperature_max-Temperature_min)))
        print("  Total inference time: {:.4f}s ({:.2f} min)".format(total_inference_time, total_inference_time/60))
        print("  Average batch time: {:.4f}s".format(avg_batch_time))
        print("  Total batches: {}".format(len(batch_times)))
        print('=' * 60)
