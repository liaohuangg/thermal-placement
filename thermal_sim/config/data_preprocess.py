...
# newdata_dir = os.path.join(script_dir, 'newdata')
newdata_dir = os.path.join(script_dir, 'data')
...
node_feats = genfromtxt(os.path.join(data_dir, 'Power_{}_{}.csv'.format(0,0)), delimiter=',')
node_labels = genfromtxt(os.path.join(data_dir, 'Temperature_{}_{}.csv'.format(0,0)), delimiter=',')

edge_feats = genfromtxt(os.path.join(data_dir, 'Edge_{}_{}.csv'.format(0, 0)), delimiter=',')
...
for i in range(Num_layout):
    for j in range(Num_power):
        node_feats = genfromtxt(os.path.join(data_dir, 'Power_{}_{}.csv'.format(i,j)), delimiter=',')
        node_labels = genfromtxt(os.path.join(data_dir, 'Temperature_{}_{}.csv'.format(i,j)), delimiter=',')

        edge_feats = genfromtxt(os.path.join(data_dir, 'Edge_{}_{}.csv'.format(i,j)), delimiter=',')

        Power = open(os.path.join(newdata_dir, 'Power_{}_{}.csv'.format(i, j)), 'w')
        totalPower = open(os.path.join(newdata_dir, 'totalPower_{}_{}.csv'.format(i,j)),'w')
        Power_sum = 0
        for m in range(node_feats.shape[0]):
            if m > 12291:
                node_feats[m][1] = 0.1
        ...
        Temperature =  open(os.path.join(newdata_dir, 'Temperature_{}_{}.csv'.format(i, j)), 'w')
        
        for m in range(node_labels.shape[0]):
            Temperature.write(str(m) + "," + str(node_labels[m][1]) + "\n")
        Temperature.close()
            
        Edge = open(os.path.join(newdata_dir, 'Edge_{}_{}.csv'.format(i, j)), 'w')
        for m in range(edge_feats.shape[0]):
            if edge_feats[m][0] in range(12288, 12300) and edge_feats[m][1] in range(12288, 12300):
                edge_feats[m][2] = 0.1        
...
with open(os.path.join(data_dir, 'MaxMinValues.csv'), 'w') as MaxMinFile:
...
with open(os.path.join(newdata_dir, 'MaxMinValues.csv'), 'w') as MaxMinFile:
            MaxMinFile.write("tPower_max, tPower_min, Power_max,Power_min,Temperature_max,Temperature_min,Conductance_max,Conductance_min\n")
            MaxMinFile.write(str(tPower_max)+","+str(tPower_min)+","+str(Power_max) + "," + str(Power_min) + "," + str(Temperature_max) + "," + str(Temperature_min) + "," + str(Conductance_max) + "," + str(Conductance_min) + "\n")
...
testFile = open(os.path.join(data_dir, 'test_data.csv'), 'w')
trainFile = open(os.path.join(data_dir, 'train_data.csv'), 'w')
...
testFile = open(os.path.join(newdata_dir, 'test_data.csv'), 'w')
trainFile = open(os.path.join(newdata_dir, 'train_data.csv'), 'w')
